...
extern struct drive_opt drives_opt[];
static void mini2440_start(void *opaque)
{
    struct mini2440_board_s *s = (struct mini2440_board_s *) opaque;
    int32_t image_size;
	...
	if (s->boot_mode == BOOT_NOR ) {
	   	image_size = load_image_targphys(drives_opt[0].file, 0x0,get_image_size(drives_opt[0].file));
		if (image_size > 0) {
	   		mini2440_printf("loaded %s (size %x)\n", drives_opt[0].file, image_size);
	    }
		else{
			printf("loaded fail,Image size=%d\n",image_size);
		}
		return;
	}
	/*
